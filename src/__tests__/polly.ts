...
const redactSensitiveString = (input: string) =>
  input
    .replace(/\b(?:sk|pk|rk)_(?:test|live)_[A-Za-z0-9_]+\b/g, "STRIPE_KEY_REDACTED")
    .replace(/\bwhsec_[A-Za-z0-9_]+\b/g, "STRIPE_WEBHOOK_SECRET_REDACTED")
    .replace(/\bpi_[A-Za-z0-9]+_secret_[A-Za-z0-9]+\b/g, "stripe-client-token-redacted")
...
const removeBlacklistedVariables = (
  ...
  if ("client_secret" in obj) {
    obj.client_secret = "test-client-token";
  }
    ...
    polly.server
      ...
      .on("beforePersist", (_req, recording: PollyRecording) => {
        ...
        recording.response.headers = recording.response.headers.filter(
          (el: Record<string, string>) => !HEADERS_BLACKLIST.has(el.name.toLowerCase()),
        );
        ...
        if (recording.request.postData?.text) {
          ...
          recording.request.postData.text = redactSensitiveString(
        ...
        if (recording.response.content?.text) {
          ...
          recording.response.content.text = redactSensitiveString(
