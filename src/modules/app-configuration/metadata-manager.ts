...
export async function deletePrivateMetadata(client: Client, keys: string[]) {
  if (!keys.length) {
    return;
  }
...
export async function deletePublicMetadata(client: Client, keys: string[]) {
  if (!keys.length) {
    return;
  }
  ...
  const { error: mutationError } = await client
    .mutation(DeletePublicMetadataDocument, {
      id: appId,
      keys,
    })
    .toPromise();
...
export const createPrivateSettingsManager = (client: Client) => {
  ...
  return new EncryptedMetadataManager({
    ...
    deleteMetadata: (keys) => deletePrivateMetadata(client, keys),
...
export const createPublicSettingsManager = (client: Client) => {
  return new MetadataManager({
    ...
    deleteMetadata: (keys) => deletePublicMetadata(client, keys),
...
export const createWebhookPrivateSettingsManager = (
  ...
  return new EncryptedMetadataManager({
    ...
    deleteMetadata: () => Promise.resolve(),
  }) as BrandedEncryptedMetadataManager;
};
