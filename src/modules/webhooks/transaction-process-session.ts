...
export const TransactionProcessSessionWebhookHandler = async (
  ...
  try {
    stripePaymentIntent = await updateStripePaymentIntent({
    ...
    if (message.includes("status of succeeded")) {
      // Payment already succeeded — fetch the intent instead of updating it
      const { getStripeApiClient } = await import("../stripe/stripe-api");
      const stripe = getStripeApiClient(stripeConfig.secretKey);
      stripePaymentIntent = await stripe.paymentIntents.retrieve(event.transaction.pspReference);
    } else {
      throw err;
    }
  }
